...
class camera():
    ...
    def rotate_remain_horizontal(self, rotation):
        if not self.lock:
            about_pos = self.pos
        else:
            about_pos = self.pos - self.offset
        ...
        glTranslate(-about_pos.x, -about_pos.y, -about_pos.z)
        glRotate(-rotation[0], self.orient.m11, self.orient.m12, 0)
        glTranslate(about_pos.x, about_pos.y, about_pos.z)
        ...
        self.orient = self.orient.rotate_legacy_remain_horizontal(rotation)
